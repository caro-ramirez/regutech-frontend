...
export default function ConfiguracionNormativa() {
  ...
  useEffect(() => {
    apiGet("/configuracion").then((c) => {
      setSlaCritica(c.sla_critica_dias);
  ...
  const handleGuardarConfig = async () => {
    if (!slaCritica || !slaAlta || !slaMedia || !slaBaja || !umbral) {
    ...
    try {
      await apiPut("/configuracion", {
        slaCriticaDias: Number(slaCritica),
      ...
      <div className="rt-panel p-4">
        ...
        <div className="row g-2 mb-2">
          <div className="col-md-2 col-6">
            <label className="form-label small" style={{ color: "var(--rt-critica)" }}>SLA Crítica (días)</label>
            <input type="number" className="form-control form-control-sm rt-input" value={slaCritica} onChange={(e) => setSlaCritica(e.target.value)} />
          </div>
          <div className="col-md-2 col-6">
            <label className="form-label small" style={{ color: "var(--rt-alta)" }}>SLA Alta (días)</label>
            <input type="number" className="form-control form-control-sm rt-input" value={slaAlta} onChange={(e) => setSlaAlta(e.target.value)} />
          </div>
          <div className="col-md-2 col-6">
            <label className="form-label small" style={{ color: "var(--rt-media)" }}>SLA Media (días)</label>
            <input type="number" className="form-control form-control-sm rt-input" value={slaMedia} onChange={(e) => setSlaMedia(e.target.value)} />
          </div>
          <div className="col-md-2 col-6">
            <label className="form-label small" style={{ color: "var(--rt-baja)" }}>SLA Baja (días)</label>
        ...
        <div className="d-flex flex-column gap-2">
          {normas.map((n) => (
            <div
              ...
              className="d-flex align-items-center justify-content-between p-2 rounded-3 rt-card-hover"
          ...
          <div className="d-flex flex-column gap-2 mb-3">
            {items.map((it) => (
              <div key={it.id_item} className="d-flex justify-content-between small py-2" style={{ borderBottom: "1px solid var(--rt-border)" }}>
                ...
                <span className={`rt-badge ${claseSeveridad(it.criticidad)}`}>{it.criticidad}</span>
              </div>
            ))}
